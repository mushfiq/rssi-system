...
public class SerialComm implements gnu.io.SerialPortEventListener {
	...
	public boolean openSio(int aPortListNumber, int aBaud) {
		...
		while (portList.hasMoreElements()) {
			...
			if (aPortListNumber == 0)
				break;
			aPortListNumber--;
		}
		// System.out.println("Oeffnen von Port Nr "+aPortListNumber);
		if (portId == null) {/* System.out.println("Port nicht gefunden"); */
			return false;
		}
		;
		...
		if (serialPort != null)
			try {
				serialPort.setSerialPortParams(aBaud, SerialPort.DATABITS_8, SerialPort.STOPBITS_1,
