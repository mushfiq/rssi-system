...
/**
 * DataProcessorRunnable checks if the batchSignalQueue is empty. If it is not empty, it takes a batch of signal
 * strengths and performs algorithm calculations on them. After that, it puts calculated values into the
 * calculatedPositionsQueue. If the queue is empty, thread sleeps for constant number of milliseconds, before checking
 * if a new batch has arrived into the batchSignalQueue.
 */
	...
	while (running == true) {
		...
		for (Map.Entry<Integer, HashMap<Integer, Double>> entry : allSignalStrengths.entrySet()) {
		    ...
		    if (position == null) {
			    ...
			    long currentTime = System.currentTimeMillis(); // TODO: Be aware that this has to be like this: long currentTime = System.currentTimeMillis();
