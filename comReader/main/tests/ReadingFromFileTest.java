...
public final class ReadingFromFileTest {
	...
	public static void main(String[] args) {
		...
		/* We instantiate the application simply by calling its "getApplication" method, 
		 ...
		 * For testing purposes, we are reading sample data from a file. 
		 * To read data from COM port, the following line of code should be changed to:
		 * 
		 ...
		 * To change the algorithm, "setAlgorithm" method of Controller class should be called: 
		 * 
		 * Application.getApplication().getController().setAlgorithm(new MyNewAlgorithm(roomMap, receivers));
		 * */
		
		/*
		 * DataProcessor class takes the signal strengths from the queue, calculates the 
		 * actual position using one of the algorithms and puts the result (actual position in form of (X,Y))
		 * into the positions queue. After that, actual positions from this queue are sent to the server. 
		 * 
		 * */
